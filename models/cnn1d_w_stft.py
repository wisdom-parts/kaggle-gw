...
class STFTInputLayer(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out_A = out[:, :, :, 0]
        out_B = out[:, :, :, 1]
        out = torch.multiply(out_A, out_A) + torch.multiply(out_B, out_B)
        # Separate the batch and channel dimensions again.
        out = out.view(x.shape[0], 3, 2049, 65)
        out = self.bn(out) # (600, 3, 2049, 65)
        return out
...
class Cnn1dSTFT(nn.Module):
    ...
    def __init__(self, hp: Cnn1dSTFTHp, apply_final_activation: bool):
        ...
        if hp.inputlayer:
            input_layer_class = STFTInputLayer
